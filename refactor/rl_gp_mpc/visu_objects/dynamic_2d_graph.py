...
class LivePlotSequential:
	...
	def update(self, obs, action, cost, iter_info:IterationInformation=None):
		...
		self.axes[2].fill_between(idxs,
			...
			self.mean_costs_std_pred[:(self.num_points_show + 1)] * MUL_STD_BOUNDS,
			self.mean_costs_pred[:(self.num_points_show + 1)] +
			self.mean_costs_std_pred[:(self.num_points_show + 1)] * MUL_STD_BOUNDS,
...
def anim_plots_2d_p(queue, num_steps_total, step_pred, obs_space, action_space, use_constraints=False, state_min=None, state_max=None, time_between_updates=0.5):
