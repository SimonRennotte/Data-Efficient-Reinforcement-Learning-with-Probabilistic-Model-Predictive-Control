...
from rl_gp_mpc.control_objects.memories.gp_memory import GpMemory
...
class GpMpcController(BaseControllerObject):
    def __init__(self, observation_low:np.array, observation_high:np.array, action_low:np.array, action_high:np.array, config:Config):
        ...
        self.memory = GpMemory(self.config.memory, dim_input=self.transition_model.dim_input, dim_state=self.transition_model.dim_state, include_time_model=self.transition_model.config.include_time_model)
    ...
    def get_action(self, obs_mu:np.array, obs_var:np.array=None, random:bool=False):
        ...
        with torch.no_grad():
            ...
            self.iter_info = IterationInformation(
                                iteration=self.actions_mapper.n_iter_ctrl,
                                state=self.states_mu_pred[0],
                                predicted_states=self.states_mu_pred[1:],
                                predicted_states_std=states_std_pred[1:],
                                predicted_actions=actions_model.detach().numpy(),
                                cost=-reward.item(),
                                cost_std=reward_var.sqrt().item(),
                                predicted_costs=-self.rewards_trajectory[1:],
                                predicted_costs_std=self.rewards_traj_var[1:].sqrt(),
                                mean_predicted_cost=np.min([-self.rewards_trajectory[1:].mean().item(), 3]),
                                mean_predicted_cost_std=self.rewards_traj_var[1:].sqrt().mean().item(),
                                lower_bound_mean_predicted_cost=self.cost_traj_mean_lcb.item()
                            )
            self.store_iter_info(self.iter_info)
            return next_action_raw

    def _get_optimal_actions(self, state_mu:torch.Tensor, state_var:torch.Tensor) -> torch.Tensor:
        x_mem, y_mem = self.get_x_y_memory()
        with torch.no_grad():
    ...
    def add_memory(self, obs: np.array, action: np.array, obs_new:np.array, reward:float, predicted_state:np.array=None, predicted_state_std:np.array=None):
        ...
        self.memory.add(state_mu, action_model, state_mu_new, reward, predicted_state=predicted_state, predicted_state_std=predicted_state_std)

        if self.memory.len_mem % self.config.training.training_frequency == 0 and \
    ...
    def compute_mean_lcb_trajectory(self, actions_mpc:np.array, obs_mu:torch.Tensor, obs_var:torch.Tensor):
        ...
        states_mu_pred, states_var_pred = self.transition_model.predict_trajectory(actions_model, obs_mu, obs_var, len_horizon=self.config.controller.len_horizon, current_time_idx=self.memory.n_iter_obs)
    ...
    def get_x_y_memory(self):
        if len(self.memory.idxs_mem_gp) > 0:
            x_mem = self.memory.x[self.memory.idxs_mem_gp]
            y_mem = self.memory.y[self.memory.idxs_mem_gp]
        else: # If no datapoints, initialize with dummy 0 variable to avoid errors. Only use for iteration info
            x_mem = torch.zeros((1, self.transition_model.dim_input))
            y_mem = torch.zeros((1, self.transition_model.dim_state))
        return x_mem, y_mem
