...
class LivePlotClassParallel:
	def __init__(self, num_steps_total, observation_space, action_space, constraints,
		...
		if use_thread:
			...
			self.graph_p = threading.Thread(target=run_graph_parallel, args=(self.pqueue, num_steps_total, step_pred, num_std_bounds,
				observation_space, action_space, constraints, time_between_updates, fontsize))
		else:
			self.pqueue = multiprocessing.Queue()
			self.graph_p = multiprocessing.Process(target=run_graph_parallel, args=(self.pqueue, num_steps_total, step_pred, num_std_bounds,
				observation_space, action_space, constraints, time_between_updates, fontsize))
			# graph_p.daemon = True
		self.graph_p.start()  # Launch reader_proc() as a separate python process
